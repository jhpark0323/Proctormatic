...
describe('useRegisterStore 상태 및 기능 테스트', () => {
  ...
  it('이름 유효성 검사가 올바르게 동작하는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());

    act(() => result.current.validateName('홍길동'));
    expect(result.current.nameError).toBe(false);

    act(() => result.current.validateName('홍'));
    expect(result.current.nameError).toBe(true);
  });

  it('이메일 유효성 검사가 올바르게 동작하는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());

    act(() => result.current.validateEmail('test@example.com'));
    expect(result.current.emailError).toBe(false);

    act(() => result.current.validateEmail('invalid-email'));
    expect(result.current.emailError).toBe(true);
  });

  it('비밀번호 유효성 검사가 올바르게 동작하는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());

    act(() => result.current.validatePassword('ValidPass123!'));
    expect(result.current.passwordError).toBe(false);

    act(() => result.current.validatePassword('short'));
    expect(result.current.passwordError).toBe(true);
  });

  it('비밀번호 확인 유효성 검사가 올바르게 동작하는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());
    ...
    act(() => result.current.validateConfirmPassword('Mismatch123!'));
    expect(result.current.confirmPasswordError).toBe(true);
  });

  it('전체 약관 동의 상태가 올바르게 설정되는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());

    act(() => result.current.handleAllCheck(true));
    ...
    act(() => result.current.handleAllCheck(false));
  ...
  it('필수 약관 개별 동의 상태가 올바르게 업데이트되는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());

    act(() => result.current.handleRequiredCheck(0, true));
    expect(result.current.requiredChecks[0]).toBe(true);

    act(() => result.current.handleRequiredCheck(1, false));
    expect(result.current.requiredChecks[1]).toBe(false);
  });

  it('마케팅 동의 상태가 올바르게 설정되는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());

    act(() => result.current.handleMarketingCheck(true));
    expect(result.current.marketingChecked).toBe(true);

    act(() => result.current.handleMarketingCheck(false));
    expect(result.current.marketingChecked).toBe(false);
  });

  it('모든 필드가 유효한 경우 true를 반환하는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());
  ...
  it('필드가 유효하지 않을 때 false를 반환하는지 확인', () => {
    const { result } = renderHook(() => useRegisterStore());
